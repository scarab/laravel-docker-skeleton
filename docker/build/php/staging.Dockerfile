...
COPY --chown=${USERNAME}:${USERNAME} application $APP_HOME/
COPY --chown=${USERNAME}:${USERNAME} .env.$ENV $APP_HOME/.env

# install all PHP dependencies
RUN if [ "$BUILD_ARGUMENT_ENV" = "dev" ] || [ "$BUILD_ARGUMENT_ENV" = "test" ]; \
    then COMPOSER_CACHE_READ_ONLY=1 COMPOSER_MEMORY_LIMIT=-1 composer -v install --optimize-autoloader --no-interaction --no-progress; \
    else COMPOSER_CACHE_READ_ONLY=1 COMPOSER_MEMORY_LIMIT=-1 composer install --optimize-autoloader --no-interaction --no-progress --no-dev; \
    fi
